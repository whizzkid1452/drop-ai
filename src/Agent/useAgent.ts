...
import { useWebLLM } from '@/Agent/useWebLLM';
import { useTrackStore } from '@/stores/useTrackStore';
import { useAudioCommand } from '@/AudioEngine/logics';
import { handleAIResponse } from '@/Agent/utils/aiResponseHandler';
import {
  createUserMessage,
  createAssistantMessage,
} from '@/Agent/utils/messageHelpers';
import { trackChatMessageSent } from '@/UI/analytics';
...
export function useAgent() {
  ...
  const sendMessage = async (content: string) => {
    ...
    if (message) {
      const { trackAIResponseReceived, trackPromptImprovementSession } = await import('@/UI/analytics');
