...
export const AudioCommandType = {
  ...
  SET_PLUGIN_SIDECHAIN: 'SET_PLUGIN_SIDECHAIN',
  RESTORE_PLUGIN_STATE: 'RESTORE_PLUGIN_STATE',
  SET_PLUGIN_FAVORITE: 'SET_PLUGIN_FAVORITE',
  ...
  TIME_STRETCH_SELECTED_REGIONS: 'TIME_STRETCH_SELECTED_REGIONS',
  PITCH_SHIFT_SELECTED_REGIONS: 'PITCH_SHIFT_SELECTED_REGIONS',
  ANALYZE_TRANSIENTS_SELECTED_REGIONS: 'ANALYZE_TRANSIENTS_SELECTED_REGIONS',
  BOUNCE_SELECTED_REGIONS: 'BOUNCE_SELECTED_REGIONS',
  FREEZE_SELECTED_REGIONS: 'FREEZE_SELECTED_REGIONS',
  SET_SOURCE_TAGS: 'SET_SOURCE_TAGS',
  AUDITION_SOURCE: 'AUDITION_SOURCE',
  STOP_SOURCE_AUDITION: 'STOP_SOURCE_AUDITION',
  CLEANUP_UNUSED_SOURCES: 'CLEANUP_UNUSED_SOURCES',
...
export const StrictAudioCommandSchema = z.discriminatedUnion('type', [
  ...
  z.strictObject({
    ...
    presetId: pluginMemberIdSchema.nullable().optional(),
    sidechainSourceTrackId: z.uuid('Invalid sidechain source Track ID format').nullable().optional(),
    stateBlob: z.string().max(1_000_000).nullable().optional(),
  }),
  z.strictObject({
  ...
  z.strictObject({
    type: z.literal(AudioCommandType.APPLY_PLUGIN_PRESET),
    trackId: z.uuid('Invalid track ID format'),
    instanceId: z.uuid('Invalid Plugin instance ID format'),
    presetId: pluginMemberIdSchema,
  }),
  z.strictObject({
    type: z.literal(AudioCommandType.SET_PLUGIN_SIDECHAIN),
    trackId: z.uuid('Invalid track ID format'),
    instanceId: z.uuid('Invalid Plugin instance ID format'),
    sourceTrackId: z.uuid('Invalid sidechain source Track ID format').nullable(),
  }),
  z.strictObject({
    type: z.literal(AudioCommandType.RESTORE_PLUGIN_STATE),
    trackId: z.uuid('Invalid track ID format'),
    instanceId: z.uuid('Invalid Plugin instance ID format'),
    parameterValues: z.record(pluginMemberIdSchema, pluginParameterValueSchema),
    presetId: pluginMemberIdSchema.nullable(),
    sidechainSourceTrackId: z.uuid('Invalid sidechain source Track ID format').nullable(),
    stateBlob: z.string().max(1_000_000).nullable(),
  }),
  z.strictObject({
    type: z.literal(AudioCommandType.SET_PLUGIN_FAVORITE),
    manifestId: pluginMemberIdSchema,
  ...
  z.strictObject({
    type: z.literal(AudioCommandType.TIME_STRETCH_SELECTED_REGIONS),
    stretchRatio: z.number().finite().min(0.25).max(4),
  }),
  z.strictObject({
    type: z.literal(AudioCommandType.PITCH_SHIFT_SELECTED_REGIONS),
    semitones: z.number().finite().min(-24).max(24),
  }),
  z.strictObject({
    type: z.literal(AudioCommandType.ANALYZE_TRANSIENTS_SELECTED_REGIONS),
    sensitivity: z.number().finite().positive().max(1),
  }),
  z.strictObject({ type: z.literal(AudioCommandType.BOUNCE_SELECTED_REGIONS) }),
  z.strictObject({ type: z.literal(AudioCommandType.FREEZE_SELECTED_REGIONS) }),
  z.strictObject({
    type: z.literal(AudioCommandType.SET_SOURCE_TAGS),
    sourceId: z.uuid('Invalid Source ID format'),
    tags: z.array(z.string().max(255)).max(32),
  }),
  z.strictObject({
    type: z.literal(AudioCommandType.AUDITION_SOURCE),
    sourceId: z.uuid('Invalid Source ID format'),
  }),
