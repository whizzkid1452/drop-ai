...
선택 상자, 버튼, 링크, `contenteditable` 요소에 초점이 있거나 IME 조합 중이면 전역 단축키를 실행하지 않는다. Track과
Region 대상 편집은 현재 선택 상태가 없으므로 전역 단축키에 연결하지 않는다.
...
MIDI 입력 구현은 `IMidiInput` 뒤에 두고 `createApp`에서 조립한다. Web UI는 사용자 동작으로 권한을 요청한 뒤 Note On을 기존 `AudioCommand`로 변환한다. 노트 36~39는 선택한 트랙의 루프 슬롯 1~4에 대응하고, 노트 40은 모든 루프를 정지한다. 채널은 전체 또는 1~16 중 하나를 선택한다.
