...
export function useAudioCommand() {
  ...
  const getFirstRegionId = (trackId: string): string => {
    ...
    if (track.regions.length === 0) {
      throw new Error(
        `No regions available in track ${trackId}. Please add a region first.`
      );
    }
  ...
  const getFirstRegionUrl = (trackId: string): string => {
    ...
    if (track.regions.length === 0) {
      throw new Error(
        `No regions available in track ${trackId}. Please add a region with URL first.`
      );
    }
    ...
    if (!url) {
      throw new Error(
        `No URL available in the first region. Please add a region with URL first.`
      );
    }
   ...
   */
  const execute = useCallback(async (command: AudioCommand): Promise<void> => {
    const service = AudioService.getInstance();
      ...
      switch (command.type) {
        ...
        case AudioCommandType.UNLOAD_REGION: {
          ...
          const regionId: string =
            command.regionId ?? getFirstRegionId(trackId);
