...
/**
 * Helper functions for command execution (can be used outside React)
 */
 ...
 */
function getFirstTrackId(): string {
...
/**
 * Execute an AudioCommand (Pure function, can be used outside React)
 * 
 * This is the core command execution logic that can be reused by:
 * - useAudioCommand (React Hook)
 * - PushController (Non-React class)
 * - Any other command executor
 * 
 * @param command - The command to execute
 */
export async function executeAudioCommand(command: AudioCommand): Promise<void> {
  const service = AudioService.getInstance();
  ...
  try {
    switch (command.type) {
      ...
      default:
        console.warn('[executeAudioCommand] Unknown command:', command);
    }
  } catch (error) {
    console.error('[executeAudioCommand] Execution failed:', error);
    throw error;
  }
}
 ...
 * - Uses executeAudioCommand internally for consistency.
 */
export function useAudioCommand() {
  const execute = useCallback(
    async (command: AudioCommand): Promise<void> => {
      return executeAudioCommand(command);
    },
    []
  );
