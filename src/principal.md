...
- 아키텍처 규칙: [`layers/architecture.md`](layers/architecture.md)
...
- 프로젝트 내부에 Ardour라는 단어를 사용하지 않는다.
...
[`layers/shared/types/audioCommand.schema.ts`](layers/shared/types/audioCommand.schema.ts)에 다음 내용을 추가한다.
...
[`layers/commands/command-executor.ts`](layers/commands/command-executor.ts)의 분기에 Command를 등록한다.
...
새 의존성이 필요하면 [`layers/apps/create-app.ts`](layers/apps/create-app.ts)에서 한 번 생성하고 주입한다.
...
- [`layers/apps/cli/index.ts`](layers/apps/cli/index.ts)의 명령 목록에 이름, 설명, 사용법을 등록한다.
- CLI 인자를 검증한 뒤 `AudioCommand`를 만들어 `CommandExecutor`에 전달한다.
- Controller를 직접 호출하는 새 CLI 명령을 만들지 않는다.

#### Agent Chat

- [`layers/apps/web/hooks/agent/useAgent/utils/getSystemPrompt.ts`](layers/apps/web/hooks/agent/useAgent/utils/getSystemPrompt.ts)의 Commands 목록에 등록한다.
...
하나의 테스트는 하나의 동작만 검증한다. 커버리지 수치보다 핵심 로직, 경계값, 에러 경로를 검증하는 의미 있는 테스트를 우선한다.
...
- `pnpm test`: 테스트 실행
- `pnpm test:watch`: 테스트 워치 모드 실행
- `pnpm lint`: ESLint 실행
- `pnpm build`: TypeScript 프로젝트 빌드와 Vite 프로덕션 빌드 실행
...
- 기능 구현 전에 실패하는 테스트를 먼저 작성하고 Red → Green → Refactor 순서로 진행한다.
- 구현 코드를 수정하면 `pnpm test`로 기존 테스트의 회귀 여부를 확인한다.
- 외부 의존성은 Mock 또는 Stub으로 대체해 단위 테스트를 격리한다.
