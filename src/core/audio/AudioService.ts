...
export class AudioService {
  ...
  get exportStartTime() {
    return this.store.getState().exportStartTime;
  }
  get exportEndTime() {
    return this.store.getState().exportEndTime;
  }
  get tempo() {
  ...
  async addRegion(
    ...
    if (trackPlayers.has(regionData.id)) {
      console.log(
        '[AudioService] Player already exists for region',
        regionData.id
      );
      return;
    }
    ...
    return new Promise((resolve, reject) => {
      const player = new Tone.Player({
        ...
        onload: () => {
          ...
          console.log('[AudioService] Region added to track', {
            trackId,
  ...
  async splitRegion(trackId: string, splitTime: number): Promise<void> {
    ...
    if (!region) {
      console.warn(
        `[AudioService] No region found at ${splitTime} on track ${trackId}`
      );
      return;
    }
  ...
  async exportProject(options?: ExportOptions): Promise<Blob> {
    ...
    if (
      !exportRange &&
      snapshot.exportStartTime !== null &&
      snapshot.exportEndTime !== null
    ) {
      exportRange = {
